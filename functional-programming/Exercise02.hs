...
eval d (Def var x y) = case eval d x of 
  Left _ -> Left "div by zero"
  Right k -> eval (define d var k) y

evalOp :: EDict -> (Double -> Double -> b) -> Expr -> Expr -> Either [Char] b
evalOp d op x y = case (eval d x, eval d y) of 
  (Right m, Right n) -> Right (op m n)
  (Left "undefined var x", Right _) -> Left "undefined var x"
  (Left "undefined var y", Right _) -> Left "undefined var y"
  (Right _, Left "undefined var x") -> Left "undefined var x"
  (Right _, Left "undefined var y") -> Left "undefined var y"
  _                                 -> Left "div by zero"
